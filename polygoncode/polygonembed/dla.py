...
from polygonembed.dla_common import conv1x1, conv1x1_block, conv3x3_block, conv7x7_block
from polygonembed.dla_resnet import ResBlock, ResBottleneck
from polygonembed.dla_resnext import ResNeXtBottleneck
...
class DLAInitBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        # x: shape (N, mid_channels, H, W)
        x = self.conv1(x)
        # x: shape (N, mid_channels, H, W)
        x = self.conv2(x)
        # x: shape (N, out_channels, (H+1)/2, (W+1)/2)
        x = self.conv3(x)
        return x
