...
class NoneSpatialRelationEncoder(nn.Module):
    """
    ...
    def __init__(self, spa_embed_dim, coord_dim = 2, ffn = None, device = "cpu"):
        ...
        """
        super(NoneSpatialRelationEncoder, self).__init__()
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim
        # assert spa_embed_dim == coord_dim
    ...
    def forward(self, coords):
        ...
        spr_embeds =  torch.FloatTensor(coords).to(self.device)
...
class KDeltaSpatialRelationEncoder(nn.Module):
    """
    Given a list of (X, Y), encode them using the position encoding function
    Here, they are a list of polygon vertices with shape (batch_size, num_context_pt, coord_dim)
    We add some (deltaX, deltaY) between point Pi with point P(i+k/2)  (k/2 points between each other)
                             and between point Pi with point P(i-k/2)  (k/2 points between each other)
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, k_delta = 1, ffn = None, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
            ...
            max_radius: the largest context radius this model can handle
        """
        super(KDeltaSpatialRelationEncoder, self).__init__()
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim
        # assert spa_embed_dim == coord_dim*(k_delta+1)
        assert k_delta >= 1
    ...
    def make_input_embeds(self, coords, k_delta):
        if type(coords) == np.ndarray:
            assert self.coord_dim == np.shape(coords)[2]
            coords = list(coords)
        elif type(coords) == list:
        ...
        left_delta = (k_delta+1)//2
        right_delta = (k_delta)//2
        coords_mats = [coords_mat]
        for start_idx in range(1, left_delta+1):
            coords_mat_delta = np.concatenate([coords_mat[:, start_idx:, :], coords_mat[:, :start_idx, :]], axis = 1)
            coords_mat_delta -= coords_mat
            coords_mats.append(coords_mat_delta)
        for end_idx in range(-right_delta, 0):
            coords_mat_delta = np.concatenate([coords_mat[:, end_idx:, :], coords_mat[:, :end_idx, :]], axis = 1)
            coords_mat_delta -= coords_mat
            coords_mats.append(coords_mat_delta)
        
        # spr_embeds: shape (batch_size, num_context_pt, coord_dim*(k_delta+1))
        spr_embeds = np.concatenate(coords_mats, axis = -1)

        assert self.spa_embed_dim == spr_embeds.shape[-1]
        return spr_embeds
    ...
    def forward(self, coords):
        ...
        Return:
            sprenc: Tensor shape (batch_size, num_context_pt, spa_embed_dim)
        """
        spr_embeds = self.make_input_embeds(coords, k_delta = self.k_delta)
...
class SphereGirdNormSpatialRelationEncoder(nn.Module):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
    ...
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, 
        ...
        Args:
            ...
            max_radius: the largest context radius this model can handle
        """
        super(SphereGirdNormSpatialRelationEncoder, self).__init__()
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim 
        self.frequency_num = frequency_num
        self.freq_init = freq_init
...
class GridNoNormCellSpatialRelationEncoder(nn.Module):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
    ...
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, 
        ...
        Args:
            ...
            max_radius: the largest context radius this model can handle
        """
        super(GridNoNormCellSpatialRelationEncoder, self).__init__()
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim 
        self.frequency_num = frequency_num
        self.freq_init = freq_init
...
class RFFSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, rbf_kernal_size = 1.0, extent = None, 
            ffn=None):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
    ...
    def make_input_embeds(self, coords):
        ...
        # coords_mat: shape (batch_size, num_context_pt, 2)
        coords_mat = torch.FloatTensor(coords_mat)
