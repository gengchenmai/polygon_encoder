...
class Trainer():
    ...
    def load_triple_dataset_dataloader(self, triple_gdf, pgon_gdf, 
        ...
        for split in ["TRAIN", "TEST", "VALID"]:
            ...
            if split == "VALID" and dup_test:
                ...
                if pgon_flag == "simple":
                    triple_dataset[split] = TripleDataset(triple_gdf = triple_split_gdf, 
                                                        pgon_gdf = pgon_gdf, 
                                                        task = task, 
                                                        id_col = id_col, 
                                                        device = device)
    ...
    def get_num_vert(self, pgon_gdf, geom_type_list, args_num_vert = 300, pgon_flag = 'simple'):
    ...
    def load_model(self):
        self.model, self.optimizer, self.args = load_model(self.model, self.optimizer, self.model_file, device = self.args.device)
    ...
    def load_pca_matrix(self, args):
        ...
        if args.pgon_enc == "nuft_pca":
            ...
            F_dim = fx * (fy//2+1) * 1 * 2
            if args.nuft_freq_init == "fft":
                self.pca_mat_file = f'''{os.path.join(args.data_dir, args.pgon_filename).replace(".pkl", "")}-pca_{args.nuft_freqXY[0]}_{args.nuft_pca_white}.npy'''
            ...
            # F_dim = fx * fy
            # if args.nuft_freq_init == "fft":
            #     self.pca_mat_file = f'''{os.path.join(args.data_dir, args.pgon_filename).replace(".pkl", "")}-nuftifft_pca_{args.nuft_freqXY[0]}_{args.nuft_pca_white}.npy'''
            # else:
            #     self.pca_mat_file = None
